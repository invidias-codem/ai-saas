// lib/ucol/prompts/kimiCoder.ts
// Kimi K3 code generation — replaces claudeCoder.ts / geminiCoder.ts /
// openRouterCoder.ts / huggingFaceCoder.ts. Preserves the stateful debate-loop
// contract: ContextPackage in → GeneratedFile[] out, plus RefinementContext and
// DiscoveredPattern[] handoffs across review rounds.
import { nimChat } from './nimChat';
import type { ContextPackage, GeneratedFile, RefinementContext, DiscoveredPattern } from '../types';
import type { ProviderCallContext } from '@/lib/llm/providers/providerInterface';
...
function parseGeneratedFiles(text: string, componentName: string): GeneratedFile[] {
    ...
    try {
        ...
        console.error('[UCOL:Kimi] Failed to parse code JSON:', text.substring(0, 500));
        return [{
...
export const kimiCoderProvider = {
    id: 'kimi-k3',
    ...
    async generateCode(
        ...
        if (refinement && refinement.feedbackHistory.length > 0) {
            ...
            for (let i = 0; i < refinement.feedbackHistory.length; i++) {
                ...
                if (fb.originalityNotes) userPrompt += `- Originality notes: ${fb.originalityNotes}\n`;
                if (fb.suggestions.length > 0) userPrompt += `- Fix instructions:\n${fb.suggestions.map(s => `  - ${s}`).join('\n')}\n`;
                if (fb.failedCriteria.length > 0) userPrompt += `- Failed criteria: ${fb.failedCriteria.join(', ')}\n`;
            }
        ...
        const { text } = await nimChat(systemPrompt, userPrompt, {
            temperature: 0.6,
            maxTokens: 8192,
