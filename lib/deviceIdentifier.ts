 ...
 */
// In-memory fallback for Private Mode / SSR
let internalDeviceId: string | null = null;
let internalDeviceInfo: DeviceInfo | null = null;

/**
 * Generate or retrieve device ID
 * Stored in localStorage to persist across sessions
 * Falls back to in-memory if localStorage is disabled (Private Mode)
 */
export function getOrCreateDeviceId(): string {
  ...
  try {
    ...
    console.warn('[DeviceID] LocalStorage access failed (likely Private Mode). Using ephemeral ID.');
    if (!internalDeviceId) {
      internalDeviceId = `device_${Date.now()}_${generateRandomSuffix()}`;
    }
    return internalDeviceId;
