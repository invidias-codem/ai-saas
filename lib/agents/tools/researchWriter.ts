...
const ResearchWriterInputSchema = z.object({
  topic: z.string().describe("The research topic or thesis statement"),
  sections: z
    .array(z.string())
    .optional()
    .describe(
      "Optional: specific sections to include (e.g. ['Abstract', 'Introduction', 'Methodology']). " +
...
type ResearchWriterInput = z.infer<typeof ResearchWriterInputSchema>;
...
export const researchWriterTool: Tool = {
  name: "write_research_paper",
  description:
    "Write a structured research paper, white paper, technical report, or literature review on any topic. " +
  ...
  async execute(input: ResearchWriterInput, context: AgentContext): Promise<ToolResult> {
    try {
...
Begin the document now:`;
      ...
      const result = await gemini.generateStream(
        ...
        `You are an expert research writer. Produce complete, high-quality ${style} documents.`,
        {
          temperature: 0.4,
      ...
      logger.error("[researchWriterTool] Error", error);
