...
exports.promoteHighConfidenceFacts = promoteHighConfidenceFacts;
exports.processConversationFacts = processConversationFacts;
exports.getUserRecentFacts = getUserRecentFacts;
...
function stripThoughtSignature(text) {
 ...
 */
async function extractFactsFromConversation(userQuery, assistantResponse) {
    try {
...
Return empty array [] if no facts can be extracted.`;
        ...
        const result = await geminiProvider.generateStream([{ role: 'user', text: conversationText }], systemPrompt, {
        ...
        try {
            ...
            return facts.filter(fact => fact.type &&
                fact.value &&
                fact.confidence >= 0.3 &&
                fact.confidence <= 1.0);
        }
 ...
 */
async function storeExtractedFacts(userId, conversationId, facts, metadata = {}) {
    const stored = [];
    const errors = [];
    for (const fact of facts) {
        try {
            const embeddingResult = await (0, embedding_1.generateEmbeddingWithMetadata)(fact.value);
            const workspaceId = typeof metadata?.workspaceId === 'string' ? metadata.workspaceId : null;
            const scope = (0, memoryPromotion_1.determineMemoryScopeForFact)({
                type: fact.type,
 ...
 */
async function promoteHighConfidenceFacts(userId, conversationId, facts) {
    let promotedCount = 0;
    ...
    for (const memory of memories) {
        ...
        if ((0, memoryPromotion_1.shouldPromoteMemory)(promotableMemory)) {
            const success = await (0, memoryPromotion_1.promoteToUserScope)(userId, memory.id);
            if (success)
                promotedCount++;
        }
    }
 ...
 */
async function processConversationFacts(userId, conversationId, userQuery, assistantResponse, metadata = {}) {
    try {
 ...
 */
async function getUserRecentFacts(userId, limit = 20, scope) {
    try {
