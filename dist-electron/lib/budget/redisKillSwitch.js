...
exports.budgetKillSwitch = exports.RedisKillSwitch = void 0;
...
// ─── Config ───────────────────────────────────────────────────────────────────
const PER_USER_LIMIT_USD = parseFloat(process.env.LLM_BUDGET_PER_USER_USD ?? "5.00");
const GLOBAL_LIMIT_USD = parseFloat(process.env.LLM_BUDGET_GLOBAL_USD ?? "500.00");
const ENFORCEMENT_ENABLED = process.env.ENABLE_LLM_BUDGET_ENFORCEMENT === "true";
...
function getRedis() {
    ...
    try {
        _redis = new redis_1.Redis({ url, token });
        return _redis;
    }
    catch (err) {
...
// ─── Key helpers ──────────────────────────────────────────────────────────────
const userKey = (userId) => `llm_spend:${userId}`;
...
function calculateCostUSD(inputTokens, outputTokens, model) {
    ...
    let pricing = MODEL_PRICING[model] ??
        ...
        Object.entries(MODEL_PRICING).find(([k]) => normalizedModel.startsWith(k.toLowerCase()))?.[1] ??
        DEFAULT_PRICING;
...
class RedisKillSwitch {
    ...
    async checkBudget(userId) {
        ...
        if (!redis) {
            console.warn("[BudgetKillSwitch] Redis unavailable — allowing request through");
            return { allowed: true, userSpendUSD: 0, globalSpendUSD: 0 };
        }
        try {
            ...
            if (userSpendUSD >= PER_USER_LIMIT_USD) {
                console.warn(`[BudgetKillSwitch] User ${userId} over budget: $${userSpendUSD.toFixed(4)} >= $${PER_USER_LIMIT_USD}`);
                return {
                    allowed: false,
                    userSpendUSD,
                    globalSpendUSD,
            ...
            if (globalSpendUSD >= GLOBAL_LIMIT_USD) {
                console.warn(`[BudgetKillSwitch] Global budget exceeded: $${globalSpendUSD.toFixed(4)} >= $${GLOBAL_LIMIT_USD}`);
                return {
                    allowed: false,
                    userSpendUSD,
                    globalSpendUSD,
            ...
            console.warn("[BudgetKillSwitch] checkBudget failed (allowing through):", err);
            return { allowed: true, userSpendUSD: 0, globalSpendUSD: 0 };
        }
    }
    /**
     ...
     */
    async recordSpend(userId, inputTokens, outputTokens, model) {
        const costUSD = calculateCostUSD(inputTokens, outputTokens, model);
        const record = { inputTokens, outputTokens, model, costUSD };
        if (costUSD <= 0)
            return record;
        const redis = getRedis();
        if (!redis)
        ...
        try {
            ...
            console.log(`[BudgetKillSwitch] Recorded $${costUSD.toFixed(6)} for user ${userId}` +
...
exports.RedisKillSwitch = RedisKillSwitch;
// Singleton export — share across module scope in serverless functions
exports.budgetKillSwitch = new RedisKillSwitch();
