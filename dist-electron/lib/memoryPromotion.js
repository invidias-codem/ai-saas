...
exports.shouldPromoteMemory = shouldPromoteMemory;
exports.determineMemoryScopeForFact = determineMemoryScopeForFact;
exports.promoteToUserScope = promoteToUserScope;
exports.processConversationForPromotion = processConversationForPromotion;
exports.getUserProfile = getUserProfile;
exports.formatUserProfileForPrompt = formatUserProfileForPrompt;
exports.getConversationMemories = getConversationMemories;
...
const USER_PROMOTABLE_FACT_TYPES = ['personal', 'preference'];
const USER_REVIEWABLE_FACT_TYPES = ['skill', 'goal'];
function normalizeMemoryMetadata(metadata) {
    return metadata && typeof metadata === 'object' ? metadata : {};
}
function isExtractedFactType(type) {
    return ['skill', 'preference', 'goal', 'personal', 'project', 'tool'].includes(type);
}
function isDurableUserMemoryType(type) {
...
function isWorkspaceCandidateType(type) {
    return type === 'project' || type === 'tool';
}
/**
 * Check if a memory should be promoted to user scope
 */
function shouldPromoteMemory(memory) {
    if (memory.scope === 'user' || memory.scope === 'workspace')
        return false;
    if (memory.confidence < 0.9)
        return false;
    const metadata = normalizeMemoryMetadata(memory.metadata);
    if (metadata.scopeLocked === true)
        return false;
    if (metadata.allowUserPromotion === false)
        return false;
    if (metadata.scopeHint === 'workspace')
        return false;
    if (isExtractedFactType(memory.type)) {
        if (USER_PROMOTABLE_FACT_TYPES.includes(memory.type))
            return true;
    ...
    if (isDurableUserMemoryType(memory.type))
        return true;
...
function determineMemoryScopeForFact(args) {
    const { type, confidence, workspaceId } = args;
    if (type === 'personal' && confidence >= 0.9)
        return 'user';
    if (type === 'preference' && confidence >= 0.9)
        return 'user';
 ...
 */
async function promoteToUserScope(userId, memoryId) {
    try {
        const { data: memory, error: fetchError } = await supabaseClient_1.supabase
            .from('memory_bank')
            .select('metadata')
            .eq('id', memoryId)
            .eq('user_id', userId)
 ...
 */
async function processConversationForPromotion(userId, conversationId) {
    try {
        const { data: memories, error } = await supabaseClient_1.supabase
            .from('memory_bank')
            .select('*')
            .eq('user_id', userId)
 ...
 */
function formatUserProfileForPrompt(memories) {
 ...
 */
async function getConversationMemories(userId, conversationId) {
    try {
        const { data, error } = await supabaseClient_1.supabase
            .from('memory_bank')
            .select('*')
