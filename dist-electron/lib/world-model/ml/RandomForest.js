 ...
 */
function majorityVote(predictions, classNames) {
 ...
 */
function meanPrediction(predictions) {
 ...
 */
function ensembleFeatureImportances(trees) {
    const sums = {};
    for (const tree of trees) {
        const imp = DecisionTreeEngine_1.DecisionTreeEngine.featureImportances(tree);
...
class RandomForest {
    ...
    static train(examples, options = {}) {
        ...
        const treeOobIndices = [];
        for (let t = 0; t < nEstimators; t++) {
            let trainingExamples;
            let oobIndices = [];
            let bootstrapIndices = [];
            if (bootstrap) {
            ...
            const tree = DecisionTreeEngine_1.DecisionTreeEngine.train(trainingExamples, {
    ...
    static predict(model, features) {
        ...
        for (const tree of model.trees) {
            const result = DecisionTreeEngine_1.DecisionTreeEngine.predict(tree, features);
        ...
        const aggregate = model.taskType === 'classification'
     ...
     */
    static oobError(model, examples, treeOobIndices) {
        ...
        const samplePredictions = Array.from({ length: n }, () => []);
        for (let t = 0; t < model.trees.length; t++) {
            const oobIdx = treeOobIndices[t];
            for (const idx of oobIdx) {
                const result = DecisionTreeEngine_1.DecisionTreeEngine.predict(model.trees[t], examples[idx].features);
     ...
     */
    static featureImportances(model) {
