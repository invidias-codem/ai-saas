 ...
 */
function toDomainProportionVector(counts, total) {
...
class DistributionShiftDetector {
    ...
    async measureDrift() {
        try {
            ...
            for (let i = 0; i < ALL_DOMAINS.length; i++) {
                ...
                const P = [currentProp, 1 - currentProp];
                const Q = [baselineProp, 1 - baselineProp];
                ...
                const alertTriggered = jsd > JS_DIVERGENCE_THRESHOLD &&
    ...
    async checkStaleness() {
        try {
            ...
            for (const measurement of alertedDomains) {
                if (existingDomains.has(measurement.domain))
                    continue;
    ...
    async generateReport() {
        ...
        try {
            ...
            const fingerprints = (fingerprintRows ?? []);
            const totalQueries = fingerprints.length;
            // ── Compute domain windows ──
            const domainWindows = await Promise.all(ALL_DOMAINS.map(domain => this.buildDomainWindow(domain, fingerprints, totalQueries, windowStart, now)));
     ...
     */
    classifySeverity(m) {
        if (m.js_divergence > 0.4 && m.graph_staleness_hours > 168)
            return 'critical';
        if (m.js_divergence > 0.25 || m.graph_staleness_hours > 120)
            return 'high';
        if (m.js_divergence > 0.15 || m.graph_staleness_hours > 72)
            return 'medium';
        return 'low';
    }
    /**
     ...
     */
    recommendAction(severity, m) {
        if (severity === 'critical')
            return 'emergency_update';
        if (severity === 'high')
            return 'refresh_graph';
        if (severity === 'medium' && m.trend === 'surging')
            return 'add_grounding_feed';
     ...
     */
    async getGraphStalenessHours(domain) {
        try {
     ...
     */
    async buildDomainWindow(domain, all, totalQueries, windowStart, windowEnd) {
        ...
        for (const term of allTermsSet) {
            const domainsWithTerm = ALL_DOMAINS.filter(d => all.filter(r => r.domain === d).some(r => r.keywords.includes(term))).length;
     ...
     */
    async getLastGraphUpdate(domain) {
        try {
            const { data, error } = await this.supabase
                .from('wm_knowledge_nodes')
                .select('updated_at')
...
exports.DistributionShiftDetector = DistributionShiftDetector;
