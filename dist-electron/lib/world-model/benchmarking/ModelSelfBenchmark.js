...
exports.ModelSelfBenchmark = void 0;
// ─────────────────────────────────────────────
// Helpers
// ─────────────────────────────────────────────
 ...
 */
class ModelSelfBenchmark {
    // ── Scoring weights ──────────────────────────────────────────────────────
    static ACCURACY_WEIGHT = 0.4;
    static CLAIM_QUALITY_WEIGHT = 0.3;
    static GRAPH_UTIL_WEIGHT = 0.2;
    static LATENCY_WEIGHT = 0.1;
    /**
     * Default p95 latency baselines (ms) per model family.
     * Matched via partial (case-insensitive) model name lookup.
     */
    static P95_LATENCY_BASELINES = {
        'gemini-flash': 300,
        'gemini': 600,
        'claude': 1200,
        'deepseek': 800,
    ...
    async scoreResponse(params) {
        ...
        let claim_quality_score;
        if (totalClaims === 0) {
            claim_quality_score = 0.5; // Neutral — we don't penalize empty responses
        }
        else {
            const goodClaims = audit.claims.filter(c => c.verdict === 'CONFIRMED' || c.verdict === 'SUPPORTED').length;
            claim_quality_score = goodClaims / totalClaims;
        }
        // ── 3. Graph utilization ─────────────────────────────────────────────────
        let graph_utilization_score;
        if (totalClaims === 0) {
        ...
        const composite_score = ModelSelfBenchmark.ACCURACY_WEIGHT * accuracy_score +
     ...
     */
    async getModelProfile(model, domain) {
        const now = new Date();
        ...
        try {
            ...
            for (const r of results) {
                ...
                hallucinatedClaims += r.audit.claims.filter(c => c.verdict === 'CONTRADICTED' || c.verdict === 'MISATTRIBUTED').length;
            }
    ...
    async getLeaderboard(domain) {
        try {
            ...
            const profiles = await Promise.all(uniqueModels.map(m => this.getModelProfile(m, domain)));
            return profiles
                .filter((p) => p !== null)
...
exports.ModelSelfBenchmark = ModelSelfBenchmark;
