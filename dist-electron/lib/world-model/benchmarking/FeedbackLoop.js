...
const ModelStore_1 = require("../ml/ModelStore");
const ModelSelfBenchmark_1 = require("./ModelSelfBenchmark");
// ─────────────────────────────────────────────
// Domain numeric encoding
// ─────────────────────────────────────────────
/** Maps domain strings to numeric feature values for routing training examples. */
const DOMAIN_NUMERIC_MAP = {
    code: 0,
    reasoning: 1,
    research: 2,
    current_events: 3,
...
class FeedbackLoop {
    // ── Demotion thresholds ───────────────────────────────────────────────────
    static DEMOTION_THRESHOLD = 0.35;
    static REINSTATE_THRESHOLD = 0.55;
    static MIN_BENCHMARKS_FOR_DEMOTION = 10;
    // ── Routing weight bounds ─────────────────────────────────────────────────
    static ROUTING_WEIGHT_FLOOR = 0.1;
    static ROUTING_WEIGHT_CEILING = 2.0;
    supabase;
    store;
    benchmark;
    /**
     ...
     */
    constructor(supabase, modelStore) {
        this.supabase = supabase;
        this.store = modelStore;
        this.benchmark = new ModelSelfBenchmark_1.ModelSelfBenchmark(supabase);
    }
    // ─────────────────────────────────────────────
    // Public API
    // ─────────────────────────────────────────────
    ...
    async runCycle() {
        ...
        for (const [key, groupResults] of groupMap) {
            ...
            const upserted = await this.upsertRoutingWeight(model, domain, routingWeight, finalDemoted, demotionOutcome.reason);
            if (upserted)
                routingWeightsAdjusted++;
    ...
    checkDemotion(profile) {
        ...
        if (profile.avg_composite_score < FeedbackLoop.DEMOTION_THRESHOLD &&
     ...
     */
    computeRoutingWeight(profile) {
        if (profile.demoted) {
            return FeedbackLoop.ROUTING_WEIGHT_FLOOR;
        }
        const raw = profile.avg_composite_score / 0.7;
        return Math.max(FeedbackLoop.ROUTING_WEIGHT_FLOOR, Math.min(FeedbackLoop.ROUTING_WEIGHT_CEILING, raw));
    }
    /**
     ...
     */
    convertToTrainingExample(result) {
     ...
     */
    async fetchRoutingWeight(model, domain) {
        try {
            const { data, error } = await this.supabase
     ...
     */
    async upsertRoutingWeight(model, domain, routingWeight, demoted, demotionReason) {
        try {
            const { error } = await this.supabase
                .from('wm_model_routing_weights')
                .upsert({
     ...
     */
    async writeTrainingExample(result) {
        ...
        await ModelStore_1.ModelStore.saveTrainingExample('routing_model', featureVector, example.label, 'auto_confirmed', example.outcome_score);
    }
}
