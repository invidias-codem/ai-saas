...
exports.calculateContextRelevance = calculateContextRelevance;
exports.rankMemoriesIntelligently = rankMemoriesIntelligently;
exports.buildUserPreferences = buildUserPreferences;
exports.buildUserPreferencesWithImport = buildUserPreferencesWithImport;
exports.filterFactsByScope = filterFactsByScope;
exports.getHighConfidenceFactsIntelligent = getHighConfidenceFactsIntelligent;
exports.recordFeedback = recordFeedback;
exports.trackFactUsage = trackFactUsage;
exports.getUserPreferences = getUserPreferences;
exports.saveUserPreferences = saveUserPreferences;
exports.synthesizeContextWithReasoning = synthesizeContextWithReasoning;
 ...
 */
function calculateContextRelevance(fact, currentConversationContext) {
 ...
 */
function rankMemoriesIntelligently(facts, vectorSimilarities, currentContext, userPreferences = {}) {
    const scored = facts.map((fact) => {
        ...
        const sentimentBoost = (userPreferences.sentimentPreference || 0) * (fact.sentiment || 0) * 0.15;
        ...
        const finalScore = vectorScore * 0.4 + // Vector similarity: 40%
 ...
 */
function buildUserPreferences(conversationHistory, existingPreferences) {
    ...
    if (avgUserLength < 100 && avgAssistantLength < 500)
        preferredDepth = 'brief';
    if (avgAssistantLength > 1500)
 ...
 */
async function buildUserPreferencesWithImport(userId, conversationHistory, existingPreferences) {
 ...
 */
function filterFactsByScope(facts, scope = 'all') {
    if (scope === 'all')
        return facts;
 ...
 */
function getHighConfidenceFactsIntelligent(facts, currentContext, userPreferences, threshold = 0.7) {
 ...
 */
async function recordFeedback(db, userId, factId, feedback) {
 ...
 * Tracks fact usage
 */
async function trackFactUsage(db, userId, factId) {
    const factRef = db.collection('users').doc(userId).collection('facts').doc(factId);
    const fact = await factRef.get();
    if (!fact.exists)
        return;
 ...
 */
async function saveUserPreferences(db, userId, preferences) {
    const prefRef = db.collection('users').doc(userId).collection('settings').doc('preferences');
    await prefRef.set({
        ...preferences,
        updatedAt: new Date(),
 ...
 */
async function synthesizeContextWithReasoning(facts, query) {
    if (facts.length === 0)
        return "";
