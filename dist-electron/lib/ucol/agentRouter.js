...
exports.getAgentRouter = getAgentRouter;
...
const proceduralMemory_1 = require("./proceduralMemory");
/** Task types where confidence can override the static model target */
const CONFIDENCE_OVERRIDABLE = new Set([
    'quick_answer',
    'quality_analysis',
    'deep_reasoning',
    'unknown',
...
class AgentRouter {
    ...
    twinRouterUrl;
    constructor() {
        this.gemini = new gemini_1.GeminiProvider();
    ...
    async classify(task) {
        ...
        try {
            const proceduralMatch = await (0, proceduralMemory_1.findMatchingProcedure)(task.userId, task.query);
            if (proceduralMatch &&
                ...
                const proceduralTaskType = proceduralMatch.record.taskType ?? 'unknown';
                const staticTarget = ROUTING_TABLE[proceduralTaskType] ?? 'gemini-flash';
                console.log(`[AgentRouter] ⚡ Procedural memory fast-path for "${task.query.substring(0, 60)}"` +
        ...
        const memorySignal = task.memoryFacts && task.memoryFacts.length > 0
            ? (0, confidenceScoring_1.scoreContextForRouting)(task.memoryFacts, task.confidenceStrategy ?? 'minimum')
            : null;
        // Classify task type via Gemini Flash
        let taskType = 'unknown';
        let classifierConfidence = confidenceScoring_1.DEFAULT_BASE_CONFIDENCE;
        let classifierReasoning = 'Unclassified — defaulting to Gemini Flash';
        try {
            const result = await this.gemini.generateStream([{ role: 'user', text: `Query: ${task.query}${task.context ? `\n\nContext: ${task.context.substring(0, 500)}` : ''}` }], CLASSIFIER_SYSTEM_PROMPT, { model: 'gemini-3.1-flash-lite-preview', temperature: 0.1, maxTokens: 256 });
        ...
        if (memorySignal && CONFIDENCE_OVERRIDABLE.has(taskType) && !isToolNode(staticTarget)) {
            ...
            const confidenceTarget = CONFIDENCE_TIER_TO_NODE[memorySignal.recommendedTier] ?? staticTarget;
            ...
            if (overrideApplied) {
                ...
                const direction = confidenceRank > staticRank
                ...
                console.log(`[AgentRouter] Confidence override: ${staticTarget} → ${confidenceTarget} (${direction})`);
            }
            return {
                taskType,
    ...
    // ─── Dispatch to JKlaw ───────────────────────────────────────────────
    async dispatchToJKlaw(task, decision, waitForResponse = false) {
        if (!this.jklawKey) {
        ...
        try {
            ...
            if (this.twinRouterUrl) {
                try {
                    ...
                    const twinTimeout = setTimeout(() => twinController.abort(), waitForResponse ? 30000 : 6000);
                    ...
                    if (twinRes.ok) {
                        ...
                        console.log(`[AgentRouter] Direct twin dispatch OK — ` +
                    ...
                    console.warn(`[AgentRouter] Twin router returned ${twinRes.status} — falling back to internal JKlaw endpoint`);
                }
                catch (twinErr) {
    ...
    async route(task) {
        ...
        const confLabel = decision.memorySignal
            ...
            : '';
        console.log(`[AgentRouter] "${task.query.substring(0, 60)}" → ${decision.targetNode}` +
 ...
 */
async function classifyQuery(query, context, memoryFacts, userId = 'system') {
