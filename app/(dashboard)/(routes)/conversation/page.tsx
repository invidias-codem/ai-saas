...
import { Paperclip, AlertCircle, SendHorizontal, X, Plus, ArrowDown } from "lucide-react";
...
const RenderTableAsChart = ({ node, ...props }: any) => {
  ...
  return (
    <div className="my-4 w-full overflow-x-auto rounded-lg border">
      <table {...props} className="w-full min-w-[500px] border-collapse text-sm" />
    </div>
  );
};
            ...
            <div key={index} className={cn(
              ...
              <div className={cn(
                ...
                <div className={cn(
                  ...
                  msg.role === "user"
                    ? "bg-secondary text-secondary-foreground rounded-[20px] rounded-tr-sm px-5 py-3 shadow-sm" // Claude-style user bubble
